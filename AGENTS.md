...
`server/scripts/linux/` 下每个 `.sh` 都是课程源码，不是只能靠猜的黑盒命令。每个脚本文件头必须说明：职责、所属边界、输入/输出、生命周期/所有权和明确不负责的事情。

每个 Shell 函数或入口前必须说明它解决的具体问题、参数和退出约定；关键命令前必须解释 WHY，尤其是：

- `set -euo pipefail` 如何让失败、未定义变量和管道错误向上传播；
- `cd`/脚本目录推导如何消除调用者当前目录和机器绝对路径依赖；
- `curl` 的 `-fL`、重试和临时文件如何下载固定第三方快照；
- `tar`/`zip` 解压时外层目录、临时目录和原子移动的含义；
- `.pinned-tag`、`.pinned-commit`、版本清单和运行时哈希分别证明什么；
- `make`、`cmake`、`cc`、`gdb`、`exec`、`nohup`、PID 文件、锁和 `SIGTERM` 的生命周期边界；
- 环境变量何时只对子进程生效，何时会进入每个 Service 的 Lua State。

第三方源码快照统一使用 `curl` + 固定 tag/commit 下载；不使用 `git clone` 作为初始化方式，也不在脚本中保留第三方 Git 历史。二进制发行包同样使用 `curl` 下载。脚本只允许在明确的本地源码开发场景使用 Git 命令，不能让两套依赖初始化语义并存。

首次出现的 Shell 概念必须同时在对应课程正文解释；代码注释负责说明当前命令的 WHY，正文负责建立可迁移的工程理解。不能用“执行以下命令”替代输入、输出、失败条件和验证方式。
